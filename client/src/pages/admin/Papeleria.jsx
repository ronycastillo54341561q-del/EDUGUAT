...
// Fecha local YYYY-MM-DD (toISOString es UTC y en GT da el día siguiente
// después de las 6pm local).
const hoyStr = () => {
  const d = new Date();
...
const nuevoDraft = () => ({
  ...
  descripcion: '', fecha: '', total: '', observaciones: '', no_deposito: '',
});

const tieneDatos = (d) =>
  !!(d.no_recibo || d.alumno_id || d.descripcion || d.fecha || d.total || d.observaciones || d.no_deposito);
...
export default function Papeleria() {
  ...
  const [busqueda,   setBusqueda]   = useState(() => ls('pap_busq', ''));
  // null = ir a la última página al cargar (mostrar el registro más reciente).
  ...
  useEffect(() => { localStorage.setItem('pap_busq', busqueda); }, [busqueda]);
  ...
  const cargar = useCallback(async () => {
    ...
    try {
      const [{ data: recs }, { data: alus }, { data: cierre }, { data: lista }] = await Promise.all([
        API.get('/papeleria'),
        API.get('/alumnos'),
        API.get('/cierres/hoy?modulo=papeleria').catch(() => ({ data: null })),
        API.get('/cierres?modulo=papeleria').catch(() => ({ data: [] })),
      ]);
      setRegistros(recs);
      setAlumnos(alus);
      setCierreHoy(cierre);
  ...
  const recargarCierre = useCallback(async () => {
    try {
      const [{ data }, { data: lista }] = await Promise.all([
        API.get('/cierres/hoy?modulo=papeleria'),
        API.get('/cierres?modulo=papeleria').catch(() => ({ data: [] })),
      ]);
      setCierreHoy(data);
      setCierres(lista);
  ...
  const paginaActual = pagina == null
    ? totalPag
    : Math.min(Math.max(1, pagina), totalPag);
    ...
    try {
      if (hasChanges) {
        ...
        setRegistros(prev => prev.map(r => {
          ...
          return {
            ...
            descripcion:      ch.descripcion  !== undefined ? ch.descripcion  : r.descripcion,
      ...
      if (hayDrafts) {
        ...
        for (const d of draftsConDatos) {
          const { data: newRec } = await API.post('/papeleria', {
  ...
  // ── Subtotales por fecha (excluye anulados) ──
  const rowMeta = (() => {
    const groups = {};
  ...
  const confirmarAnular = async () => {
    ...
    setAnulandoMsg('');
    try {
      await API.post(`/papeleria/${anulando.id}/anular`);
      setRegistros(prev => prev.map(r => r.id === anulando.id ? {
        ...r,
        anulado: 1,
        descripcion: (r.descripcion || '').toString().toUpperCase().startsWith('ANULADO')
          ? r.descripcion
          : `ANULADO${r.descripcion ? ' — ' + r.descripcion : ''}`,
  ...
  const descriptionCambios = (() => {
    ...
    if (hayDrafts) partes.push(`${draftsConDatos.length} nuevo(s) registro(s)`);
    return partes.join(' + ');
  })();
      ...
      <div className="admin-content">
        ...
        <div className="asist-filtros">
          <input
            ...
            placeholder="Buscar por alumno, recibo o descripción..."
            value={busqueda}
                    ...
                    {visibles.map((r, idx) => {
                      ...
                      return (
                        <tr key={r.id} style={rowStyle}>
                          ...
                          <td>
                            <input className="recibo-input" type="text"
                              value={getVal(r, 'descripcion')}
                              onChange={e => setEdit(r.id, 'descripcion', e.target.value)}
                    ...
                    {esUltimaPag && drafts.map((d) => (
                      <tr key={d._key} style={{ background: '#f3f8ff', borderTop: '2px solid #c5cae9' }}>
                        ...
                        <td>
                          <input className="recibo-input" type="text"
                            value={d.descripcion}
                            onChange={e => updateDraft(d._key, { descripcion: e.target.value })}
                            placeholder="Descripción..." />
                        </td>
                        <td>
                          <input className="recibo-input" type="date"
